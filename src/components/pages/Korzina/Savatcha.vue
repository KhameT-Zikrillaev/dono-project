          ...
          <div v-for="item in CounterStore.oneuser?.tovar" :key="item.id"
            ...
            <div class="savatcha-info-left-con-1 flex">
              ...
              <div class="flex flex-col justify-between ml-[20px]">
                <h3 class="text-[16px] p768:text-[18px] font-bold">{{ item.name }}</h3>

                <div class="flex w-[100px] p768:w-[150px] h-[40px] rounded-md justify-between">
                  <button @click="minus(item)" class="w-[25%] bg-[#F77F00] text-[30px] flex items-center justify-center text-white rounded-l-md">-</button>
                  <span class="w-[50%] flex items-center justify-center font-semibold border-t border-b border-[#C4C4C4]">{{ item.amount }} шт</span>
                  <button @click="increase(item)" class="w-[25%] bg-[#F77F00] text-[30px] flex items-center justify-center text-white rounded-r-md">+</button>
                </div>
              </div>
            </div>
            ...
            <button @click="deletproduct(item)" class="absolute right-[1%] top-[1%]">X</button>
          </div>
        </div>
        ...
        <div class="savatcha-info-right p-[20px] rounded-[5px] bg-white shadow-md p992:max-w-[370px] p992:min-h-[330px] h-full w-full flex flex-col">
          <div class="flex-grow-1">
            ...
            <div class="flex justify-between items-center text-[18px] text-[#666666]">
              ...
              <span>{{ CounterStore.totalprice }} so’m</span>
            </div>
          </div>
          <RouterLink to="/buyurtma" @click="zakaz" class="bg-[#F77F00] rounded-[6px] ml-auto mt-[25px] p992:ml-[0px] p992:mt-auto p-[11px] text-[23px] p480:max-w-[330px] w-full text-white text-center font-bold">
            Buyurtma berish
          </RouterLink>
        </div>
      </div>
...
const notifyXato = () => {
  toast.error("Savatcha bo'sh", { autoClose: 1000 });
};

if (!CounterStore.oneuser || !CounterStore.oneuser.tovar) {
  router.push('/');
}
...
function minus(item) {
  if (item.amount === 1) {
    ...
    item.totalsum = parseFloat(item.price.replace(/\s+/g, '').replace(',', '.')) * item.amount;
  }
  // Обновляем общую цену и количество
  CounterStore.totalproductprice();
  CounterStore.totalbasketnumb();
}
...
// Обеспечиваем, что обновления правильно отслеживаются
watch(() => CounterStore.totalprice, (newValue) => {
  console.log(`Total Price updated: ${newValue}`);
});

watch(() => CounterStore.totalbasketamount, (newValue) => {
  console.log(`Total Basket Amount updated: ${newValue}`);
});
</script>
